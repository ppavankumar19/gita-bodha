  ...
  return (
    ...
    >
      ...
      <div className="relative p-5 sm:p-8">
        {/* Top row */}
        <div className="flex items-start justify-between mb-4 sm:mb-5 flex-wrap gap-2">
          <div className="flex items-center gap-2">
            <span className="font-ui text-white/90 text-[9px] sm:text-[10px] font-bold bg-white/20 px-2.5 sm:px-3 py-1 rounded-full uppercase tracking-widest backdrop-blur-sm">
              🌞 Daily Sloka
            </span>
            <span className="font-ui text-white/70 text-[9px] sm:text-[10px]">{today}</span>
          </div>
        ...
        {/* Sloka text */}
        <p className="sloka-text text-white text-base sm:text-lg whitespace-pre-line mb-3 drop-shadow-sm leading-relaxed">
          {sloka.sloka_sanskrit?.split('\n').slice(0, 2).join('\n')}
        </p>

        {/* Chapter & verse badge */}
        <p className="font-ui text-white/80 text-[10px] sm:text-xs mb-3 sm:mb-4 font-medium">
          Adhyayam {sloka.chapter} · Slokam {sloka.verse}
        </p>

        {/* Child summary */}
        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 sm:p-4 mb-4 sm:mb-5 border border-white/10">
          <p className="font-telugu text-white text-sm sm:text-base font-bold leading-relaxed">
        ...
        <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-5 sm:mb-6">
          {sloka.moral_themes
            .filter(t => !/[\u0C00-\u0C7F]/.test(t))
            .map(t => (
              <span key={t} className="font-ui text-[9px] sm:text-[10px] bg-white/20 text-white px-2.5 sm:px-3 py-1 rounded-full backdrop-blur-sm font-semibold uppercase tracking-wider">
                {t}
              </span>
            ))}
        </div>
